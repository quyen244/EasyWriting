...
def mae(pred: list[float], gold: list[float]) -> float:
    return mean(abs(p - g) for p, g in zip(pred, gold, strict=True))


def rmse(pred: list[float], gold: list[float]) -> float:
    return math.sqrt(mean((p - g) ** 2 for p, g in zip(pred, gold, strict=True)))
...
def bias(pred: list[float], gold: list[float]) -> float:
    ...
    return mean(p - g for p, g in zip(pred, gold, strict=True))
...
def within(pred: list[float], gold: list[float], tol: float) -> float:
    ...
    if not pred:
        return 0.0
    return sum(abs(p - g) <= tol + 1e-9 for p, g in zip(pred, gold, strict=True)) / len(pred)
...
def spearman_rho(pred: list[float], gold: list[float]) -> float:
    ...
    num = sum((a - mp) * (b - mg) for a, b in zip(rp, rg, strict=True))
