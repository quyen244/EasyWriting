...
from src.models import AssessmentResult, EssaySubmission, SubmissionStatus
...
class TestPersistence:
    def test_a_scored_submission_and_its_result_are_stored(
        self, scoring_client, authed_client_factory, db_session
    ) -> None:
        _, token, account = authed_client_factory("flow.persist@example.com")
        ...
        submission = db_session.get(EssaySubmission, submission_id)
        assert submission is not None
        assert submission.status is SubmissionStatus.SCORED
        assert submission.essay_text == TASK_2_ESSAY
        assert str(submission.user_id) == account["id"]

        result = (
            db_session.query(AssessmentResult)
            .filter(AssessmentResult.submission_id == submission.id)
            .one()
        )
