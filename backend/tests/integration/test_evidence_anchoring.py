...
class TestQuoteAttribution:
    ...
    async def test_quotes_reach_the_api_response_intact(
        self, override_llm, authed_client_factory
    ) -> None:
        _, token, _ = authed_client_factory("evidence.api@example.com")
        client = override_llm(
            FakeLLMClient(
                band=5.5,
                quotes_by_criterion={"TASK_RESPONSE": [OFF_TOPIC_QUOTE]},
        ...
        response = client.post(
            "/api/v1/assessments",
            json={"task_type": "TASK_2", "essay_text": ESSAY_WITH_PLANTED_DEFECTS},
            headers={"Authorization": f"Bearer {token}"},
        )
        assert response.status_code == 201, response.text

        criteria = {c["criterion"]: c for c in response.json()["criteria"]}
        assert OFF_TOPIC_QUOTE in criteria["TASK_RESPONSE"]["evidence_quotes"]
