...
| [002-core-app-ux](./002-core-app-ux/) | **Planned — Phase 1 design complete** | The **landing page alone** (`/`), grounded in the real `writewise` Figma design. Existing `frontend/landing/*` components are rewritten in place, not rebuilt from scratch (research.md R1). No backend, no API, no database — every entity is static content. **General Training scope decision still flagged, not resolved** — this plan builds against the spec's documented default. `tasks.md` still stale pending `/speckit-tasks`. |
