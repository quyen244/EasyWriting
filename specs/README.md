...
| [002-core-app-ux](./002-core-app-ux/) | **Active — narrowed to landing page, rewritten 2026-08-21** | No longer the four-surface bundle — now the **landing page alone** (`/`), grounded in the real `writewise` Figma design (fileKey `JGr2ZuKKC8JEiAIHzNAFLH`) instead of the retired Stitch mockups. FAQ, workspace, and profile no longer have a spec home here — see Planned below. **One product-scope decision flagged, not yet resolved**: the design references General Training, which the grader (001) doesn't support yet. `plan.md`/`tasks.md`/etc. are stale pending `/speckit-plan`. |
